...
$sqlhor1 = "SELECT * FROM tbl_template_hor_1 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID' ";
$queryhor1  = mysqli_query($conn, $sqlhor1);

$sqlhor1_public = "SELECT * FROM tbl_template_hor_1 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID' AND public_status = 1";
$queryhor1_public = mysqli_query($conn, $sqlhor1_public);
if($queryhor1_public == true){
$resulthor1_public = mysqli_fetch_array($queryhor1_public);
}

$sqlhor1_publicforjs = "SELECT * FROM tbl_template_hor_1 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID'";
$queryhor1_publicforjs  = mysqli_query($conn, $sqlhor1_publicforjs);
...
<body class="h-100 indexbackgroundpage">
                  ...
                  <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                      ...
                      <span class="name-template">
                        ...
                        <span class="text-mb-2">เเนวนอน รูปเเบบที่ 1</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                  <?php if($resulthor1_public == null){ ?>
                  ...
                  <div class="container-fluid margintopdisplay">
                    <div class="row justify-content-center">
                      <div class="col-lg-2 col-md-2 col-sm-2" style="background-color: #24282d; border-radius: 25px 0px 0px 25px; position: static; width: 200px; height: 550px;">
                        <?php if($resulthor1_public['image_type'] == "image") {
                                $image = explode("|", $resulthor1_public['image_slide']); ?>
                        <img src='../<?php echo $image[1]; ?>' class='img-fluid' width='100%' />
                        <?php }else{
                              $e = explode("|", $resulthor1_public['image_slide']);
                              foreach ($e as $key) {
                                if($key != ''){ ?>
                        <img class='mySlides' src='../<?php echo $key; ?>' class='img-fluid' width='100%' />
                        <?php }
                              }
                            } ?>
                      </div>
                      <div class="col-lg-6 col-md-6 col-sm-6">
                        <div class="row">
                          <div class="col-lg-12 col-md-12 col-sm-12" style="background-color: #24282d; border-radius: 0px 25px 0px 0px; position: static; width: 600px; height: 500px;">
                            <video width="100%" height="100%" style="margin-top:10px" controls>
                              <source src="../<?php echo $resulthor1_public['video']; ?>" type="video/mp4"> </video>
                          </div>
                        </div>
                        <div class="row">
                          <div class="col-lg-9 col-md-9 col-sm-9" style="background-color: #24282d;">
                            <div class="textsilde">
                              <div class="fontslide">
                                <?php echo $resulthor1_public['text_content']; ?>
                              </div>
                            </div>
                          </div>
                    ...
                    <div class="col justify-content-center">
                      <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                          <li class="page-item"><a class="page-link" href="showhor5.php">
                              <</a> </li> <li class="page-item active">
                          <li class="page-item"><a class="page-link" href="showhor.php" style="background-color: #24282d; color: white">1</a></li>
                          </li>
                          ...
                          <li class="page-item"><a class="page-link" href="showhor2.php">></a></li>
                        </ul>
                      </nav>
                    </div>
                  </div>
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover">
                        ...
                        <tbody class="table-body">
                          <?php $listloop = 0; 
                          if($queryhor1 == true){
                          while ($resulthor1 = mysqli_fetch_array($queryhor1, MYSQLI_ASSOC)) { ?>
                          <tr>
                            <th scope="row">
                              <?php echo $listloop; ?>
                            </th>
                            <td>
                              <?php echo $resulthor1['template_name']; ?>
                            </td>
                            <td>
                              <?php echo $resulthor1['upload_on']; ?>
                            </td>
                            <td>
                              <?php if($resulthor1['public_status'] == 0){ echo "ไม่เผยเเผร่"; }else{ echo "เผยเเผร่";} ?>
                            </td>
                            <td>
                              <!-- <form action="../php/select_template/select_template_hor_1.php" method="post"> -->
                                <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor1['template_id']?>" />
                                <input type="hidden" class="form-control" id="template_select_id" name="template_select_id" value="<?php echo $resulthor1['template_select_id']?>" />
                                <input type="hidden" class="form-control" id="template_name" name="template_name" value="<?php echo $resulthor1['template_name']?>" />
                                <input type="hidden" class="form-control" id="image_slide" name="image_slide" value="<?php echo $resulthor1['image_slide']?>" />
                                <input type="hidden" class="form-control" id="video" name="video" value="<?php echo $resulthor1['video']?>" />
                                <input type="hidden" class="form-control" id="text_content" name="text_content" value="<?php echo $resulthor1['text_content']?>" />
                                <input type="hidden" class="form-control" id="shop_id" name="shop_id" value="<?php echo $resulthor1['shop_id']?>" />
                                <input type="hidden" class="form-control" id="user_id" name="user_id" value="<?php echo $resulthor1['user_id']?>" />
                                <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor1['device_id']?>" />
                                <input type="hidden" class="form-control" id="public_status" name="public_status" value="<?php echo $resulthor1['public_status']?>" />
                            ...
                            <td>
                              <form name="delete_form" action="../php/unselect_template/unselect_template_hor_1.php"
                                method="post">
                                <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor1['template_id']?>" />
                                <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor1['device_id']?>" />
                                <input type="hidden" class="form-control" id="public_status" name="public_status" value="<?php echo $resulthor1['public_status']?>" />
                                <button type="submit" class="close" aria-label="Close">
                                  <?php if($resulthor1['public_status'] == 1){ echo '<i class="fas fa-ban" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                </button>
                              </form>
                            </td>
                            <td>
                              <form name="delete_form" action="../php/delete/delete_tem_hor1.php" method="post">
                                ...
                                <button type="submit" class="close" aria-label="Close" style="padding-right: 10px">
                                  <?php if($resulthor1['public_status'] == 0){ echo '<i class="fas fa-trash-alt" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                </button>
                              </form>
                            </td>
                          </tr>
  ...
  <script>
        $(document).ready(function (e) {
          ...
          <?php $listloopforjs = 0;  if($queryhor1_publicforjs == true){ while ($resulthor1_publicforjs = mysqli_fetch_array($queryhor1_publicforjs, MYSQLI_ASSOC)) { ?>


          $("#showtheme<?php echo $listloopforjs; ?>").click(function () {
            
            console.log('<?php echo $listloopforjs; ?>')
            var template_id = '<?php echo $resulthor1_publicforjs['template_id']; ?>'
            var template_select_id = '<?php echo $resulthor1_publicforjs['template_select_id']; ?>'
            var device_id = '<?php echo $resulthor1_publicforjs['device_id']; ?>'
