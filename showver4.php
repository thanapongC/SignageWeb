...
$sqlver4 = "SELECT * FROM tbl_template_ver_4 WHERE user_id = {$_SESSION["user_id"]}  AND device_id = $device_ID ";
$queryver4 = mysqli_query($conn, $sqlver4);
// if($queryver4 == true){
// $resultver4 = mysqli_fetch_array($queryver4);
// }

$sqlver4_public = "SELECT * FROM tbl_template_ver_4 WHERE user_id = {$_SESSION["user_id"]} AND device_id = $device_ID AND public_status = 1";
$queryver4_public  = mysqli_query($conn, $sqlver4_public);
if($queryver4_public == true){
$resultver4_public = mysqli_fetch_array($queryver4_public);
}else{
  $resultver4_public = null;
}

$sqlver4_publicforjs = "SELECT * FROM tbl_template_ver_4 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID'";
$queryver4_publicforjs  = mysqli_query($conn, $sqlver4_publicforjs);
...
<body class="h-100 indexbackgroundpage">
                  ...
                  <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                      ...
                      <span class="name-template">
                        ...
                        <span class="text-mb-2">เเนวตั้ง รูปเเบบที่ 4</span>
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="container-fluid margintopdisplay">
                    <div class="row justify-content-center">
                      <div class="col-lg-4 col-md-4 col-sm-4" style="background-color: #24282d; border-radius: 25px 25px 25px 25px;">
                        <?php if($resultver4_public['image_type'] == "image"){ $image = explode("|", $resultver4_public['image_slide']); ?>
                        <img src='../<?php echo $image[1]; ?>' class='img-fluid' width='100%' height="780px" style='margin-top: 10px; margin-bottom: 10px' />
                        <?php }else{ $e = explode("|", $resultver4_public['image_slide']); foreach ($e as $key) { if($key != ''){ ?>
                        <img class='mySlides' src='../<?php echo $key; ?>' class='img-fluid' width='100%' height="780px"
                      ...
                      <div class="col justify-content-center">
                        <nav aria-label="Page navigation example">
                          <ul class="pagination justify-content-center">
                            <li class="page-item"><a class="page-link" href="showver3.php">
                            ...
                            <li class="page-item"><a class="page-link" href="showver4.php" style="background-color: #24282d; color: white">4</a></li>
                            </li>
                            <li class="page-item"><a class="page-link" href="showver5.php">5</a></li>
                            <li class="page-item"><a class="page-link" href="showver5.php">></a></li>
                          </ul>
                        </nav>
                      </div>
                    </div>
                      ...
                      <div class="table-responsive">
                        <table class="table table-hover">
                          ...
                          <tbody class="table-body">
                            <?php $listloop = 0; 
                            if($queryver4 == true){
                            while ($resultver4 = mysqli_fetch_array($queryver4, MYSQLI_ASSOC)) { 
                            ?>
                            <tr>
                              <th scope="row">
                                <?php echo $listloop; ?>
                              ...
                              <td>
                                <?php echo $resultver4['template_name']; ?>
                              </td>
                              <td>
                                <?php echo $resultver4['upload_on']; ?>
                              </td>
                              <td>
                                <?php if($resultver4['public_status'] == 0){ echo "ไม่เผยเเผร่"; }else{ echo "เผยเเผร่";} ?>
                              </td>
                              <td>
                                <!-- <form action="../php/select_template/select_template_ver_4.php" method="post"> -->
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resultver4['template_id']?>" />
                                  <input type="hidden" class="form-control" id="template_select_id" name="template_select_id"
                                    value="<?php echo $resultver4['template_select_id']?>" />
                                  <input type="hidden" class="form-control" id="template_name" name="template_name"
                                    value="<?php echo $resultver4['template_name']?>" />
                                  <input type="hidden" class="form-control" id="image_slide" name="image_slide" value="<?php echo $resultver4['image_slide']?>" />
                                  <input type="hidden" class="form-control" id="video" name="video" value="<?php echo $resultver4['video']?>" />
                                  <input type="hidden" class="form-control" id="text_content" name="text_content" value="<?php echo $resultver4['text_content']?>" />
                                  <input type="hidden" class="form-control" id="shop_id" name="shop_id" value="<?php echo $resultver4['shop_id']?>" />
                                  <input type="hidden" class="form-control" id="user_id" name="user_id" value="<?php echo $resultver4['user_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resultver4['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resultver4['public_status']?>" />
                              ...
                              <td>
                                <form name="delete_form" action="../php/unselect_template/unselect_template_ver_4.php"
                                  method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resultver4['template_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resultver4['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                  ...
                                  <button type="submit" class="close" aria-label="Close">
                                    <?php if($resultver4['public_status'] == 1){ echo '<i class="fas fa-ban" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                              <td>
                                <form name="delete_form" action="../php/delete/delete_tem_ver4.php" method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resultver4['template_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resultver4['public_status']?>" />
                                  <button type="submit" class="close" aria-label="Close" style="padding-right: 10px">
                                    <?php if($resultver4['public_status'] == 0){ echo '<i class="fas fa-trash-alt" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                            </tr>
  ...
  <script>
        $(document).ready(function (e) {
          ...
          <?php $listloopforjs = 0;  if($queryver4_publicforjs == true){ while ($resultver4_publicforjs = mysqli_fetch_array($queryver4_publicforjs, MYSQLI_ASSOC)) { ?>


          $("#showtheme<?php echo $listloopforjs; ?>").click(function () {
            
            console.log('<?php echo $listloopforjs; ?>')
            var template_id = '<?php echo $resultver4_publicforjs['template_id']; ?>'
            var template_select_id = '<?php echo $resultver4_publicforjs['template_select_id']; ?>'
            var device_id = '<?php echo $resultver4_publicforjs['device_id']; ?>'
          ...
          function uploadFormData(formData) {
            $.ajax({
              url: "../php/select_template/select_template_ver_4.php",
              type: "POST",
              data: formData,
              contentType: false,
              cache: false,
