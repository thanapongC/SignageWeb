...
$sqlhor5 = "SELECT * FROM tbl_template_hor_5 WHERE user_id = {$_SESSION["user_id"]} AND device_id = $device_ID ";
$queryhor5  = mysqli_query($conn, $sqlhor5);
// if($queryhor5 == true){
// $resulthor5 = mysqli_fetch_array($queryhor5);
// }

$sqlhor5_public = "SELECT * FROM tbl_template_hor_5 WHERE user_id = {$_SESSION["user_id"]} AND device_id = $device_ID AND public_status = 1";
$queryhor5_public  = mysqli_query($conn, $sqlhor5_public);
if($queryhor5_public == true){
$resulthor5_public = mysqli_fetch_array($queryhor5_public);
}else{
  $resulthor5_public = null;
}

$sqlhor5_publicforjs = "SELECT * FROM tbl_template_hor_5 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID'";
$queryhor5_publicforjs  = mysqli_query($conn, $sqlhor5_publicforjs);
...
<body class="h-100 indexbackgroundpage">
                  ...
                  <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                      ...
                      <span class="name-template">
                        ...
                        <span class="text-mb-2">เเนวนอน รูปเเบบที่ 5</span>
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="container-fluid margintopdisplay">
                    <div class="row justify-content-center">
                      <div class="col-lg-6 col-md-6 col-sm-6" style="margin: 0;position: static; width: 600px; height: 500px; background-color: #24282d; border-radius: 25px 0px 0px 0px;">
                        <?php if($resulthor5_public['image_type_1'] == "image"){ $image = explode("|", $resulthor5_public['image_slide_1']); ?>
                        <img src='../<?php echo $image[1]; ?>' class='img-fluid' width='730px' height="530px" style='margin-top: 5px' />
                        <?php }else{ $e = explode("|", $resulthor5_public['image_slide_1']); foreach ($e as $key) { if($key != ''){ ?>
                        <img class='mySlides' src='../<?php echo $key; ?>' class='img-fluid' width='730px' height="530px"
                      ...
                      <div class="col-lg-2 col-md-2 col-sm-2" style="margin: 0;position: static; width: 200px; height: 500px; background-color: #24282d; border-radius: 0px 25px 0px 0px;">
                        <?php if($resulthor5_public['image_type_2'] == "image"){ $image2 = explode("|", $resulthor5_public['image_slide_2']); ?>
                        <img src='../<?php echo $image2[1]; ?>' class='img-fluid' width='210px' height="530px" style='margin-top: 5px' />
                        <?php }else{  $e2 = explode("|", $resulthor5_public['image_slide_2']); foreach ($e2 as $key2) { if($key2 != ''){ ?>
                        <img class='mySlides2' src='../<?php echo $key2; ?>' class='img-fluid' width='210px' height="530px"
                          style='margin-top: 10px' />
                        <?php } } } ?>
                        <script>
                          var myIndex2 = 0;
                      ...
                      <div class="col-lg-8 col-md-8 col-sm-8" style="background-color: #24282d; border-radius: 0px 0px 25px 25px;">
                        <div class="row">
                          <div class="col-lg-9 col-md-9 col-sm-9">
                            <div class="textsilde">
                              <div class="fontslide">
                                <?php echo $resulthor5_public['text_content']; ?>
                              </div>
                            </div>
                          </div>
                      ...
                      <div class="col justify-content-center">
                        <nav aria-label="Page navigation example">
                          <ul class="pagination justify-content-center">
                            <li class="page-item"><a class="page-link" href="showhor4.php">
                            ...
                            <li class="page-item"><a class="page-link" href="showhor5.php" style="background-color: #24282d; color: white">5</a></li>
                            </li>
                            <li class="page-item"><a class="page-link" href="showhor.php">></a></li>
                          </ul>
                        </nav>
                      </div>
                    </div>
                      ...
                      <div class="table-responsive">
                        <table class="table table-hover">
                          ...
                          <tbody class="table-body">
                            <?php $listloop = 0; 
                            if($queryhor5 == true){
                            while ($resulthor5 = mysqli_fetch_array($queryhor5, MYSQLI_ASSOC)) { 
                            ?>
                            <tr>
                              <th scope="row">
                                <?php echo $listloop; ?>
                              ...
                              <td>
                                <?php echo $resulthor5['template_name']; ?>
                              </td>
                              <td>
                                <?php echo $resulthor5['upload_on']; ?>
                              </td>
                              <td>
                                <?php if($resulthor5['public_status'] == 0){ echo "ไม่เผยเเผร่"; }else{ echo "เผยเเผร่";} ?>
                              </td>
                              <td>
                                <!-- <form action="../php/select_template/select_template_hor_5.php" method="post"> -->
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor5['template_id']?>" />
                                  <input type="hidden" class="form-control" id="template_select_id" name="template_select_id"
                                    value="<?php echo $resulthor5['template_select_id']?>" />
                                  <input type="hidden" class="form-control" id="template_name" name="template_name"
                                    value="<?php echo $resulthor5['template_name']?>" />
                                  <input type="hidden" class="form-control" id="image_slide" name="image_slide" value="<?php echo $resulthor5['image_slide']?>" />
                                  <input type="hidden" class="form-control" id="video" name="video" value="<?php echo $resulthor5['video']?>" />
                                  <input type="hidden" class="form-control" id="text_content" name="text_content" value="<?php echo $resulthor5['text_content']?>" />
                                  <input type="hidden" class="form-control" id="shop_id" name="shop_id" value="<?php echo $resulthor5['shop_id']?>" />
                                  <input type="hidden" class="form-control" id="user_id" name="user_id" value="<?php echo $resulthor5['user_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor5['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resulthor5['public_status']?>" />
                              ...
                              <td>
                                <form name="delete_form" action="../php/unselect_template/unselect_template_hor_5.php"
                                  method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor5['template_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor5['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                  ...
                                  <button type="submit" class="close" aria-label="Close">
                                    <?php if($resulthor5['public_status'] == 1){ echo '<i class="fas fa-ban" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                              <td>
                                <form name="delete_form" action="../php/delete/delete_tem_hor5.php" method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor5['template_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resulthor5['public_status']?>" />
                                  <button type="submit" class="close" aria-label="Close" style="padding-right: 10px">
                                    <?php if($resulthor5['public_status'] == 0){ echo '<i class="fas fa-trash-alt" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                            </tr>
  ...
  <script>
        $(document).ready(function (e) {
          ...
          <?php $listloopforjs = 0;  if($queryhor5_publicforjs == true){ while ($resulthor5_publicforjs = mysqli_fetch_array($queryhor5_publicforjs, MYSQLI_ASSOC)) { ?>


          $("#showtheme<?php echo $listloopforjs; ?>").click(function () {
            
            console.log('<?php echo $listloopforjs; ?>')
            var template_id = '<?php echo $resulthor5_publicforjs['template_id']; ?>'
            var template_select_id = '<?php echo $resulthor5_publicforjs['template_select_id']; ?>'
            var device_id = '<?php echo $resulthor5_publicforjs['device_id']; ?>'
          ...
          function uploadFormData(formData) {
            $.ajax({
              url: "../php/select_template/select_template_hor_5.php",
              type: "POST",
              data: formData,
              contentType: false,
              cache: false,
