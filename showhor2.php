...
$sqlhor2 = "SELECT * FROM tbl_template_hor_2 WHERE user_id = {$_SESSION["user_id"]} AND device_id = $device_ID ";
$queryhor2  = mysqli_query($conn, $sqlhor2);
// if($queryhor2 == true){
// $resulthor2 = mysqli_fetch_array($queryhor2);
// }

$sqlhor2_public = "SELECT * FROM tbl_template_hor_2 WHERE user_id = {$_SESSION["user_id"]} AND device_id = $device_ID AND public_status = 1";
$queryhor2_public = mysqli_query($conn, $sqlhor2_public);
if($queryhor2_public == true){
$resulthor2_public = mysqli_fetch_array($queryhor2_public);
}else{
  $resulthor2_public = null;
}

$sqlhor2_publicforjs = "SELECT * FROM tbl_template_hor_2 WHERE user_id = {$_SESSION["user_id"]} AND device_id = '$device_ID'";
$queryhor2_publicforjs  = mysqli_query($conn, $sqlhor2_publicforjs);
...
<body class="h-100 indexbackgroundpage">
                  ...
                  <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                      ...
                      <span class="name-template">
                        ...
                        <span class="text-mb-2">เเนวนอน รูปเเบบที่ 2</span>
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="container-fluid margintopdisplay">
                    <div class="row justify-content-center">
                        ...
                        <div class="row">
                          <div class="col-lg-12 col-md-12 col-sm-12" style="margin: 0; position: absolute; top: 50%; -ms-transform: translateY(-50%); transform: translateY(-50%);">
                            <video width="100%" height="100%" controls>
                              <source src="../<?php echo $resulthor2_public['video']; ?>" type="video/mp4"> </video>
                          </div>
                        </div>
                      </div>
                      <div class="col-lg-2 col-md-2 col-sm-3 col-2" style="position: static; width: 200px; height: 550px; background-color: #24282d; border-radius: 0px 25px 25px 0px;">
                        ...
                        <?php if($resulthor2_public['image_type'] == "image"){ $image = explode("|", $resulthor2_public['image_slide']); ?>
                        <img src='../<?php echo $image[1]; ?>' class='img-fluid' width='210' height="530px" style="margin-top: 10px" />
                        <?php }else{ $e = explode("|", $resulthor2_public['image_slide']); foreach ($e as $key) { if($key != ''){ ?>
                        <img class='mySlides' src='../<?php echo $key; ?>' class='img-fluid' width='210' height="530px"
                      ...
                      <div class="col justify-content-center">
                        <nav aria-label="Page navigation example">
                          <ul class="pagination justify-content-center">
                            <li class="page-item"><a class="page-link" href="showhor.php">
                                <</a> </li> <li class="page-item"><a class="page-link" href="showhor.php">1</a></li>
                            <li class="page-item active">
                            <li class="page-item"><a class="page-link" href="showhor2.php" style="background-color: #24282d; color: white">2</a></li>
                            </li>
                            ...
                            <li class="page-item"><a class="page-link" href="showhor3.php">></a></li>
                          </ul>
                        </nav>
                      </div>
                    </div>
                      ...
                      <div class="table-responsive">
                        <table class="table table-hover">
                          ...
                          <tbody class="table-body">
                            <?php $listloop = 0; 
                            if($queryhor2 == true){
                            while ($resulthor2 = mysqli_fetch_array($queryhor2, MYSQLI_ASSOC)) { 
                            ?>
                            <tr>
                              <th scope="row">
                                <?php echo $listloop; ?>
                              ...
                              <td>
                                <?php echo $resulthor2['template_name']; ?>
                              </td>
                              <td>
                                <?php echo $resulthor2['upload_on']; ?>
                              </td>
                              <td>
                                <?php if($resulthor2['public_status'] == 0){ echo "ไม่เผยเเผร่"; }else{ echo "เผยเเผร่";} ?>
                              </td>
                              <td>
                                <!-- <form action="../php/select_template/select_template_hor_2.php" method="post"> -->
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor2['template_id']?>" />
                                  <input type="hidden" class="form-control" id="template_select_id" name="template_select_id"
                                    value="<?php echo $resulthor2['template_select_id']?>" />
                                  <input type="hidden" class="form-control" id="template_name" name="template_name"
                                    value="<?php echo $resulthor2['template_name']?>" />
                                  <input type="hidden" class="form-control" id="image_slide" name="image_slide" value="<?php echo $resulthor2['image_slide']?>" />
                                  <input type="hidden" class="form-control" id="video" name="video" value="<?php echo $resulthor2['video']?>" />
                                  <input type="hidden" class="form-control" id="text_content" name="text_content" value="<?php echo $resulthor2['text_content']?>" />
                                  <input type="hidden" class="form-control" id="shop_id" name="shop_id" value="<?php echo $resulthor2['shop_id']?>" />
                                  <input type="hidden" class="form-control" id="user_id" name="user_id" value="<?php echo $resulthor2['user_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor2['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resulthor2['public_status']?>" />
                              ...
                              <td>
                                <form name="delete_form" action="../php/unselect_template/unselect_template_hor_2.php"
                                  method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor2['template_id']?>" />
                                  <input type="hidden" class="form-control" id="device_id" name="device_id" value="<?php echo $resulthor2['device_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                  ...
                                  <button type="submit" class="close" aria-label="Close">
                                    <?php if($resulthor2['public_status'] == 1){ echo '<i class="fas fa-ban" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                              <td>
                                <form name="delete_form" action="../php/delete/delete_tem_hor2.php" method="post">
                                  <input type="hidden" class="form-control" id="template_id" name="template_id" value="<?php echo $resulthor2['template_id']?>" />
                                  <input type="hidden" class="form-control" id="public_status" name="public_status"
                                    value="<?php echo $resulthor2['public_status']?>" />
                                  <button type="submit" class="close" aria-label="Close" style="padding-right: 10px">
                                    <?php if($resulthor2['public_status'] == 0){ echo '<i class="fas fa-trash-alt" style="color: #5B838D; font-size: 30px"></i>'; } ?>
                                  </button>
                                </form>
                              </td>
                            </tr>
  ...
  <script>
        $(document).ready(function (e) {
          ...
          <?php $listloopforjs = 0;  if($queryhor2_publicforjs == true){ while ($resulthor2_publicforjs = mysqli_fetch_array($queryhor2_publicforjs, MYSQLI_ASSOC)) { ?>


          $("#showtheme<?php echo $listloopforjs; ?>").click(function () {
            
            console.log('<?php echo $listloopforjs; ?>')
            var template_id = '<?php echo $resulthor2_publicforjs['template_id']; ?>'
            var template_select_id = '<?php echo $resulthor2_publicforjs['template_select_id']; ?>'
            var device_id = '<?php echo $resulthor2_publicforjs['device_id']; ?>'
          ...
          function uploadFormData(formData) {
            $.ajax({
              url: "../php/select_template/select_template_hor_2.php",
              type: "POST",
              data: formData,
              contentType: false,
              cache: false,
